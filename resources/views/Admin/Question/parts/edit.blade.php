...
<form id="form" enctype="multipart/form-data" method="POST" action="{{route('questions.update',$question->id)}}">
    @csrf
    @method('PUT')
    <div class="row mt-0">
        <x-form.input class="col-sm-12" title="القسم " :required="true">
            <select name="category_id" class="form-control form-control-solid select2" data-placeholder="اختر القسم">
                ...
                @foreach($categories as $category)
                    <option value="{{$category->id}}" {{$question->category_id == $category->id ? 'selected' : ''}}>{{$category->name_ar}}</option>
                @endforeach
            </select>
        </x-form.input>
        <x-form.input class="col-sm-12" title="السؤال ( بالعربية ) " :required="true">
            <input type="text" class="form-control form-control-solid" placeholder="السؤال ( بالعربية ) "
                   name="question_ar" value="{{$question->question_ar}}"/>
        </x-form.input>

        <x-form.input class="col-sm-12" title="السؤال ( بالانجليزية ) " :required="true">
            <input type="text" class="form-control form-control-solid" placeholder="السؤال ( بالانجليزية ) "
                   name="question_en" value="{{$question->question_en}}"/>
        </x-form.input>
        ...
        <x-form.input class="col-sm-12" title="الاجابات" :required="true">
            <select class="form-control select2" name="question_answers[]" data-placeholder="اختر الاجابات" multiple>
                @foreach($answers as $answer)
                    <option value="{{$answer->id}}" {{in_array($answer->id,$selectedQuestionAnswersIds)?'selected':''}} > {{$answer->answer_ar}} </option>
                @endforeach
            </select>
        </x-form.input>
    </div>
