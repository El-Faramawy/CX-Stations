...
window.onscroll = function () {
  var appScroll = document.documentElement.scrollTop || document.body.scrollTop;
...
function change_box_container(value) {
  // Check if '.pc-content' exists
  const pcContent = document.querySelector('.pc-content');
  const footerWrapper = document.querySelector('.footer-wrapper');
  const activeButton = document.querySelector('.theme-container .btn.active');
  if (pcContent && footerWrapper) {
    if (value === 'true') {
      pcContent.classList.add('container');
